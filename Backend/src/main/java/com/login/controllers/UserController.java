...
public class UserController {
    ...
    public ResponseEntity<OtpResponse> requestLoginOtp(@RequestBody OtpRequest request) {
        try {
            ...
            Student student = studentService.getStudentByEmail(request.getEmail());
            if (student != null && student.getMobileNumber() != null && !student.getMobileNumber().isEmpty()) {
        ...
        try {
            ...
            if (student == null) {
                return ResponseEntity.badRequest()
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(new WhatsAppResponse(false, "User not found", null, "Please register first"));
            }

            // Generate OTP
            String otp = userService.generateOtp(request.getEmail());
            userService.deleteOtp(request.getEmail());
    ...
    public ResponseEntity<?> verifyWhatsAppNumber(@RequestBody LoginRequest request) {
        try {
            ...
            if (whatsAppOtpOpt.isEmpty()) {
                return ResponseEntity.ok()
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(new WhatsAppResponse(true, "Email not found in WhatsApp OTP records", null));
            }
            ...
            if (whatsAppOtp.isExpired()) {
                return ResponseEntity.badRequest()
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(new WhatsAppResponse(false, "OTP expired", whatsAppOtp.getMobileNumber(), "Please request a new OTP"));
            }
            ...
            if (!whatsAppOtp.getOtp().equals(request.getOtp())) {
                return ResponseEntity.badRequest()
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(new WhatsAppResponse(false, "Invalid OTP", whatsAppOtp.getMobileNumber(), "The provided OTP is invalid"));
            }
            ...
            try {
                ...
                // Update mobile number from whatsapp_otp
                String mobileNumber = whatsAppOtp.getMobileNumber();
                student.setMobileNumber(mobileNumber);
                logger.info("Updating mobile number {} for student: {}", mobileNumber, request.getEmail());
                ...
                logger.error("Error updating student record: {}", e.getMessage());
                // If there's an error, still try to clean up OTP records
                try {
                    whatsAppOtpRepository.delete(whatsAppOtp);
                    // Delete OTP from users table if it exists
                    userService.deleteOtp(request.getEmail());
                } catch (Exception ex) {
                    logger.error("Error cleaning up OTP records: {}", ex.getMessage());
                }
                
                return ResponseEntity.ok()
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(new WhatsAppResponse(true, "Mobile number updated successfully", whatsAppOtp.getMobileNumber()));
            }
        } catch (Exception e) {
